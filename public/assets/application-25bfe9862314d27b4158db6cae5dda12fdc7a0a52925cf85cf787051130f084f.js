...
(function() {
  jQuery(function() {
    var locals, projetos, tasks;
    projetos = $('#relato_projeto_id').html();
    $('#relato_cliente_id').change(function() {
      var cliente, options;
      cliente = $('#relato_cliente_id :selected').text();
      options = $(projetos).filter("optgroup[label='" + cliente + "']").html();
      if (options) {
        return $('#relato_projeto_id').html(options);
      } else {
        return $('#relato_projeto_id').empty();
      }
    });
    ...
    $('#relato_cliente_id').change(function() {
      ...
      if (options) {
        return $('#relato_local_id').html(options);
      } else {
        return $('#relato_local_id').empty();
      }
    });
    tasks = $('#relato_task_id').html();
    return $('#relato_projeto_id').change(function() {
      var options, projeti;
      projeti = $('#relato_projeto_id :selected').text();
      options = $(tasks).filter("optgroup[label='" + projeti + "']").html();
      if (options) {
        return $('#relato_task_id').html(options);
      } else {
        return $('#relato_task_id').empty();
      }
    });
  });
